...
import io.github.romvoid95.oauth2.exceptions.MissingScopeException;
import io.github.romvoid95.oauth2.requests.OAuth2Action;
import io.github.romvoid95.oauth2.requests.OAuth2Requester;
import io.github.romvoid95.oauth2.requests.OAuth2URL;
import io.github.romvoid95.oauth2.session.DefaultSessionController;
import io.github.romvoid95.oauth2.session.Session;
import io.github.romvoid95.oauth2.session.SessionController;
import io.github.romvoid95.oauth2.session.SessionData;
import io.github.romvoid95.oauth2.state.DefaultStateController;
...
public class OAuth2ClientImpl implements OAuth2Client {
	...
	return OAuth2URL.AUTHORIZE.compile(clientId, EncodingUtil.encodeUTF8(redirectUri), Scope.join(scopes),
		stateController.generateNewState(redirectUri));
    }

    @Override
    public OAuth2Action<Session> startSession(String code, String state, String identifier, Scope... scopes)
	...
	return new OAuth2Action<Session>(this, Method.POST, oAuth2URL.getRouteWithBaseUrl()) {
		...
		return sessionController.createSession(new SessionData(identifier, body.getString("access_token"),
			body.getString("refresh_token"), body.getString("token_type"),
			OffsetDateTime.now().plusSeconds(body.getInt("expires_in")), scopes));
	    }
	};
    }
	...
	return new OAuth2Action<OAuth2User>(this, Method.GET, OAuth2URL.CURRENT_USER.compile()) {
		...
		return new OAuth2UserImpl(OAuth2ClientImpl.this, session, body.getLong("id"),
			body.getString("username"), body.getString("discriminator"), body.optString("avatar", null),
			body.optString("email", null), body.optBoolean("verified", false),
	...
	return new OAuth2Action<List<OAuth2Guild>>(this, Method.GET, OAuth2URL.CURRENT_USER_GUILDS.compile()) {
		...
		for (int i = 0; i < body.length(); i++) {
		    ...
		    list.add(new OAuth2GuildImpl(OAuth2ClientImpl.this, obj.getLong("id"), obj.getString("name"),
			    obj.optString("icon", null), obj.getBoolean("owner"), obj.getInt("permissions")));
		}
		return list;
	    }
	};
