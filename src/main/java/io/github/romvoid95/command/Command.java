 ...
 * <p>
 * The internal inheritance for Commands used in JDATools is that of the
 * Command object.
 * <p>
 ...
 * abstract
 * {@link io.github.romvoid95.command.Command#execute(io.github.romvoid95.command.CommandEvent)
 ...
 * wrapper called a {@link io.github.romvoid95.command.CommandEvent
 * CommandEvent} and is performed in two steps:
 * <ul>
 * <li>{@link io.github.romvoid95.command.Command#run(CommandEvent) run} - The
 ...
 * response.</li>
 * <li>{@link io.github.romvoid95.command.Command#execute(CommandEvent) execute}
 * - The command, now being cleared to run, executes and performs whatever lies
 * in the abstract body method.</li>
 * </ul>
 */
public abstract class Command {
	...
	/**
	 * The {@link io.github.romvoid95.command.Command.Category Category} of the
	 * command. <br>
	 ...
	 * {@link net.dv8tion.jda.api.entities.Guild Guild}, {@code false} if it may be
	 * used in both a Guild and a DM. <br>
	 * Default {@code true}.
	 */
	 ...
	 * to calling the {@link io.github.romvoid95.command.Command#name Command.name}.
	...
	/**
	 * The {@link io.github.romvoid95.command.Command.CooldownScope CooldownScope}
	 * of the command. This defines how far of a scope cooldowns have. <br>
	 * Default {@link io.github.romvoid95.command.Command.CooldownScope#USER
	 * CooldownScope.USER}.
	 */
	...
	protected void addAgument(CommandArgument<?> argument) {
	...
	/**
	 * The main body method of a {@link io.github.romvoid95.command.Command
	 * Command}. <br>
	 * This is the "response" for a successful
	 * {@link io.github.romvoid95.command.Command#run(CommandEvent)
	 * #run(CommandEvent)}.
	 * 
	 * @param event The {@link io.github.romvoid95.command.CommandEvent
	 * CommandEvent} that triggered this Command
	 */
	protected abstract void execute(CommandEvent event);

	/**
	 * Runs checks for the {@link io.github.romvoid95.command.Command Command} with
	 * the given {@link io.github.romvoid95.command.CommandEvent CommandEvent} that
	 * called it. <br>
	 ...
	 * <p>
	 * {-{@link io.github.romvoid95.command.Command#name name}},
	 * {-{@link io.github.romvoid95.command.Command.Category category name}}, or
	 ...
	 * {@link io.github.romvoid95.command.Command#usesTopicTags usesTopicTags} to
	 * {@code false} will cause this method to always return {@code true}, as the
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#name Command.name} for
	 * the Command.
	 *
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#help Command.help} for
	 * the Command.
	 *
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#category
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#arguments
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#requiredRoles
	 * Command.requiredRoles} for the Command.
	 *
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#cooldown
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#userPermissions
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#botPermissions
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#aliases Command.aliases}
	 * for the Command.
	 *
	...
	/**
	 * Gets the {@link io.github.romvoid95.command.Command#children
	 ...
	 * {@link io.github.romvoid95.command.CommandEvent CommandEvent}.
	 *
	 ...
	 * {@link io.github.romvoid95.command.CommandEvent CommanEvent}.
	 *
	...
	/**
	 * To be used in {@link io.github.romvoid95.command.Command Command}s as a means
	 * of organizing commands into "Categories" as well as terminate command usage
	 * when the calling {@link io.github.romvoid95.command.CommandEvent
	...
	public static class Category {
		 ...
		 * {@link io.github.romvoid95.command.Command.Category#test(io.github.romvoid95.command.CommandEvent)}
		 * returns {@code false}.
		 * 
		 * @param name The name of the Category
	...
	/**
	 * A series of {@link java.lang.Enum Enum}s used for defining the scope size for
	 * a {@link io.github.romvoid95.command.Command Command}'s cooldown.
	 * <p>
	 * The purpose for these values is to allow easy, refined, and generally
	 * convenient keys for cooldown scopes, allowing a command to remain on cooldown
	 * for more than just the user calling it, with no unnecessary abstraction or
	 * developer input.
	 * Cooldown keys are generated via
	 * {@link io.github.romvoid95.command.Command#getCooldownKey(CommandEvent)
	 * Command#getCooldownKey(CommandEvent)} using 1-2 Snowflake ID's corresponding
	 * to the name (IE: {@code USER_CHANNEL} uses the ID's of the User and the
	 * Channel from the CommandEvent).
	 * <p>
	 * However, the issue with generalizing and generating like this is that the
	 * command may be called in a non-guild environment, causing errors internally.
	 * <br>
	 * To prevent this, all of the values that contain "{@code GUILD}" in their name
	 * default to their "{@code CHANNEL}" counterparts when commands using them are
	 * called outside of a {@link net.dv8tion.jda.api.entities.Guild Guild}
	 * environment.
	 * <ul>
	 * <li>{@link io.github.romvoid95.command.Command.CooldownScope#GUILD GUILD}
	 * defaults to {@link io.github.romvoid95.command.Command.CooldownScope#CHANNEL
	 * CHANNEL}.</li>
	 * <li>{@link io.github.romvoid95.command.Command.CooldownScope#USER_GUILD
	 * USER_GUILD} defaults to
	 * {@link io.github.romvoid95.command.Command.CooldownScope#USER_CHANNEL
	 * USER_CHANNEL}.</li>
	 * </ul>
	 * These are effective across a single instance of JDA, and not multiple ones,
	 * save when multiple shards run on a single JVM and under a
	 * {@link net.dv8tion.jda.api.sharding.ShardManager ShardManager}. <br>
	 * There is no shard magic, and no guarantees for a 100% "global" cooldown,
	 * unless all shards of the bot run under the same ShardManager, and/or via some
	 * external system unrelated to JDATools.
	 *
	 * @since 1.3
	 * 
	 * @see io.github.romvoid95.command.Command#cooldownScope Command.cooldownScope
	 */
	public enum CooldownScope {
		/**
		 * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User
		 * User} across all locations on this instance (IE: TextChannels,
		 * PrivateChannels, etc).
		 * <p>
		...
		/**
		 * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User
		 * User} local to the {@link net.dv8tion.jda.api.entities.MessageChannel
		...
		/**
		 * Applies the cooldown to the {@link net.dv8tion.jda.api.entities.Guild Guild}
		...
		/**
		 * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User
		 * User} local to the {@link net.dv8tion.jda.api.entities.Guild Guild} the
		 * command is called in.
		 * <p>
		 * The key for this is generated in the format
		 * <ul>
		...
		/**
		 * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User
		 * User} on the Shard the command is called on.
		 * <p>
		 * The key for this is generated in the format
		 * <ul>
		 ...
		 * <p>
		 * As this implies: the command will be unusable on the instance of JDA in all
		 * types of {@link net.dv8tion.jda.api.entities.MessageChannel MessageChannel}s
