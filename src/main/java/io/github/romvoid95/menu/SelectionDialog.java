...
/**
 * A {@link io.github.romvoid95.menu.Menu Menu} implementation that creates a
...
public class SelectionDialog extends Menu {
	...
	SelectionDialog(EventWaiter waiter, Set<User> users, Set<Role> roles, long timeout, TimeUnit unit,
		...
		this(waiter, users, roles, timeout, unit, choices, leftEnd, rightEnd, defaultLeft, defaultRight, color, loop,
				success, cancel, text, false);
	}
	 ...
	 * {@link net.dv8tion.jda.api.entities.Message Message}, starting with the first
	 * selection.
	 * 
	...
	private Message render(int selection) {
		...
		return mbuilder
				.setEmbeds(
						new EmbedBuilder().setColor(color.apply(selection)).setDescription(sbuilder.toString()).build())
				.build();
	}

	/**
	 * The {@link io.github.romvoid95.menu.Menu.Builder Menu.Builder} for a
	 * {@link io.github.romvoid95.menu.SelectionDialog SelectuibDialog}.
	 */
	public static class Builder extends Menu.Builder<Builder, SelectionDialog> {
		...
		/**
		 * Builds the {@link io.github.romvoid95.menu.SelectionDialog SelectionDialog}
		 * with this Builder.
		 *
		...
		public SelectionDialog build() {
			...
			return new SelectionDialog(waiter, users, roles, timeout, unit, choices, leftEnd, rightEnd, defaultLeft,
					defaultRight, color, loop, selection, cancel, text, singleSelectionMode);
		}
		 ...
		 * the current selection number as determined by the provided
		 * {@link java.util.function.Function Function}. <br>
		 * As the selection changes, the Function will re-process the current selection
		 * number, allowing for the color of the embed to change depending on the
		 * selection number.
		 *
		 ...
		 * be displayed when the {@link io.github.romvoid95.menu.SelectionDialog
		...
		/**
		 * Sets the text of the {@link net.dv8tion.jda.api.entities.Message Message} to
		 * be displayed relative to the current selection number as determined by the
		 * provided {@link java.util.function.Function Function}. <br>
		 * As the selection changes, the Function will re-process the current selection
		 * number, allowing for the displayed text of the Message to change depending on
		...
		/**
		 * Sets the text to use on either side of all unselected items. This will not be
		...
		/**
		 * Sets if the Menu should exit when a selection was made. By default, this is
		...
		/**
		 * Sets a {@link java.util.function.BiConsumer BiConsumer} action to perform
		 * once a selection is made. <br>
		 * The {@link net.dv8tion.jda.api.entities.Message Message} provided is the one
		 * used to display the menu and the {@link java.lang.Integer Integer} is that of
		 * the selection made by the user, and selections are in order of addition, 1
		 * being the first String choice.
		 *
		...
		/**
		 * Sets a {@link java.util.function.Consumer Consumer} action to take if the
		 * menu is cancelled, either via the cancel button being used, or if the
		 * SelectionDialog times out.
		 *
