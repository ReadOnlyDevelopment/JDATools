package io.github.romvoid95.menu;
...
import io.github.romvoid95.waiter.EventWaiter;
...
/**
 * A {@link io.github.romvoid95.menu.Menu Menu} of ordered buttons signified by
 * numbers or letters, each with a reaction linked to it for users to click.
 * <p>
...
public class OrderedMenu extends Menu {
	...
	public final static String[] NUMBERS = new String[] { "1\u20E3", "2\u20E3", "3\u20E3", "4\u20E3", "5\u20E3",
			"6\u20E3", "7\u20E3", "8\u20E3", "9\u20E3", "\uD83D\uDD1F" };
	...
	OrderedMenu(EventWaiter waiter, Set<User> users, Set<Role> roles, long timeout, TimeUnit unit, Color color,
			String text, String description, List<String> choices, BiConsumer<Message, Integer> action,
	...
	/**
	 * Shows the OrderedMenu as a new {@link net.dv8tion.jda.api.entities.Message
	...
	public void display(MessageChannel channel) {
		...
		if (channel.getType() == ChannelType.TEXT && !allowTypedInput && !((TextChannel) channel).getGuild()
				.getSelfMember().hasPermission((TextChannel) channel, Permission.MESSAGE_ADD_REACTION))
	...
	public void display(Message message) {
		...
		// Does not allow typed input
		// Does not have permission to add reactions
		if (message.getChannelType() == ChannelType.TEXT && !allowTypedInput && !message.getGuild().getSelfMember()
	...
	private void waitGeneric(Message m) {
		...
		waiter.waitForEvent(GenericMessageEvent.class, e -> {
			// If we're dealing with a message reaction being added we return whether it's
			// valid
			if (e instanceof MessageReactionAddEvent)
				return isValidReaction(m, (MessageReactionAddEvent) e);
			// If we're dealing with a received message being added we return whether it's
			// valid
			...
			if (e instanceof MessageReactionAddEvent) {
				...
				if (event.getReaction().getReactionEmote().getName().equals(CANCEL))
					...
					// The int provided in the success consumer is not indexed from 0 to number of
					// choices - 1,
					// but from 1 to number of choices. So the first choice will correspond to 1,
					// the second
					// choice to 2, etc.
	...
	/**
	 * The {@link io.github.romvoid95.menu.Menu.Builder Menu.Builder} for an
	 * {@link io.github.romvoid95.menu.OrderedMenu OrderedMenu}.
	 */
	public static class Builder extends Menu.Builder<Builder, OrderedMenu> {
		...
		/**
		 * Builds the {@link io.github.romvoid95.menu.OrderedMenu OrderedMenu} with this
		 * Builder.
		 *
		 ...
		 * <ul>
		 * <li>No
		 * {@link io.github.romvoid95.waiter.EventWaiter
		...
		public OrderedMenu build() {
			...
			return new OrderedMenu(waiter, users, roles, timeout, unit, color, text, description, choices, selection,
					cancel, useLetters, allowTypedInput, addCancel);
		}
		...
		/**
		 * Sets the builder to build an {@link io.github.romvoid95.menu.OrderedMenu
		 * OrderedMenu} using letters for ordering and reactions (IE: A, B, C, etc.).
		 * <br>
		...
		/**
		 * If {@code true}, {@link net.dv8tion.jda.api.entities.User User}s can type the
		 * number or letter of the input to make their selection, in addition to the
		 * reaction option.
		 *
		...
		/**
		 * If {@code true}, adds a cancel button that performs the timeout action when
		 * selected.
		 *
		 ...
		 * be displayed when the {@link io.github.romvoid95.menu.OrderedMenu
		...
		/**
		 * Sets the {@link java.util.function.BiConsumer BiConsumer} action to perform
		 * upon selecting a option.
		 *
		 ...
		 * {@link io.github.romvoid95.menu.OrderedMenu OrderedMenu} is cancelled.
		 *
