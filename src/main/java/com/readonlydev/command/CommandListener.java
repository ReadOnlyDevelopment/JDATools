...
public interface CommandListener {
     ...
     *
     * @param event
     *        The MessageContextMenuEvent that triggered the MessageContextMenu
     * @param menu
     *        The MessageContextMenu that was triggered
     */
    default void onMessageContextMenu(MessageContextMenuEvent event,
     ...
     */
    default void onUserContextMenu(UserContextMenuEvent event,
            UserContextMenu menu) {}

    /**
     * Called when a {@link Command} is triggered by a {@link CommandEvent}
     * after it's completed successfully. <p>Note that a <i>successfully</i>
     * completed command is one that has not encountered an error or exception.
     * Calls that do face errors should be handled by
    ...
    /**
     * Called when a {@link SlashCommand} is triggered by a
     * {@link SlashCommandEvent} after it's completed successfully. <p>Note that
     * a <i>successfully</i> completed slash command is one that has not
     ...
     *        The SlashCommand that was triggered
     */
    default void onCompletedSlashCommand(SlashCommandEvent event,
            SlashCommand command) {}

    /**
     * Called when a {@link MessageContextMenu} is triggered by a
     * {@link MessageContextMenuEvent} after it's completed successfully.
     * <p>Note that a <i>successfully</i> completed context menu interaction is
     * one that has not encountered an error or exception. Calls that do face
     ...
     *        The MessageContextMenu that was triggered
     */
    default void onCompletedMessageContextMenu(MessageContextMenuEvent event,
            MessageContextMenu menu) {}

    /**
     * Called when a {@link UserContextMenu} is triggered by a
     * {@link UserContextMenuEvent} after it's completed successfully. <p>Note
     * that a <i>successfully</i> completed context menu interaction is one that
     * has not encountered an error or exception. Calls that do face errors
     ...
     *        The MessageContextMenu that was triggered
     */
    default void onCompletedUserContextMenu(UserContextMenuEvent event,
            UserContextMenu menu) {}
     ...
     *        The SlashCommand that was triggered
     */
    default void onTerminatedSlashCommand(SlashCommandEvent event,
            SlashCommand command) {}
     ...
     */
    default void onTerminatedMessageContextMenu(MessageContextMenuEvent event,
            MessageContextMenu menu) {}
     ...
     *        The ContextMenu that was triggered
     */
    default void onTerminatedUserContextMenu(UserContextMenuEvent event,
            UserContextMenu menu) {}

    /**
     * Called when a {@link MessageReceivedEvent} is caught by the Client
     * Listener's but doesn't correspond to a {@link Command}. <p>In other
     * words, this catches all <b>non-command</b> MessageReceivedEvents allowing
     ...
     * execution</b>. <p>This doesn't account for exceptions thrown during other
     * pre-checks, and should not be treated as such! <p>An example of this
     * misconception is via a {@link Command.Category} test:
     *
     * <pre>
     ...
     * </pre>
     * 
     * <p> The {@link NullPointerException} thrown will not be caught by this
     * method!
     *
     * @param event
     *        The CommandEvent that triggered the Command
     ...
     */
    default void onCommandException(CommandEvent event, Command command,
            Throwable throwable) {
     ...
     * execution</b>. <p>This doesn't account for exceptions thrown during other
     * pre-checks, and should not be treated as such! <p> The
     ...
     *        The Throwable thrown during Command execution
     */
    default void onSlashCommandException(SlashCommandEvent event,
            SlashCommand command, Throwable throwable) {
        // Default rethrow as a runtime exception.
        throw throwable instanceof RuntimeException
                ? (RuntimeException) throwable
                : new RuntimeException(throwable);
     ...
     * Throwable} <b>during execution</b>. <p>This doesn't account for
     * exceptions thrown during other pre-checks, and should not be treated as
     * such! <p> The {@link NullPointerException} thrown will not be caught by
     * this method!
     *
     * @param event
     ...
     *        The Throwable thrown during Command execution
     */
    default void onMessageContextMenuException(MessageContextMenuEvent event,
            MessageContextMenu menu, Throwable throwable) {
        // Default rethrow as a runtime exception.
        throw throwable instanceof RuntimeException
                ? (RuntimeException) throwable
                : new RuntimeException(throwable);
     ...
     *        The Throwable thrown during Command execution
     */
    default void onUserContextMenuException(UserContextMenuEvent event,
            UserContextMenu menu, Throwable throwable) {
        // Default rethrow as a runtime exception.
        throw throwable instanceof RuntimeException
                ? (RuntimeException) throwable
                : new RuntimeException(throwable);
