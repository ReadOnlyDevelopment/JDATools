...
/**
 * An implementable "Listener" that can be added to a {@link Client} and used to handle events relating to {@link Command}s.
 */
public interface CommandListener
{
    ...
    /**
     * Called when a {@link Command} is triggered by a {@link CommandEvent} after it's completed successfully. <p>Note that a <i>successfully</i> completed command is one that has not encountered an error or exception. Calls that do face errors should be handled by
    ...
    /**
     * Called when a {@link SlashCommand} is triggered by a {@link SlashCommandEvent} after it's completed successfully. <p>Note that a <i>successfully</i> completed slash command is one that has not encountered an error or exception. Calls that do face errors should be handled by
    ...
    /**
     * Called when a {@link MessageContextMenu} is triggered by a {@link MessageContextMenuEvent} after it's completed successfully. <p>Note that a <i>successfully</i> completed context menu interaction is one that has not encountered an error or exception. Calls that do face errors should be
     * handled by {@link CommandListener#onTerminatedMessageContextMenu(MessageContextMenuEvent, MessageContextMenu)}
     *
    ...
    /**
     * Called when a {@link UserContextMenu} is triggered by a {@link UserContextMenuEvent} after it's completed successfully. <p>Note that a <i>successfully</i> completed context menu interaction is one that has not encountered an error or exception. Calls that do face errors should be handled by
    ...
    /**
     * Called when a {@link MessageReceivedEvent} is caught by the Client Listener's but doesn't correspond to a {@link Command}. <p>In other words, this catches all <b>non-command</b> MessageReceivedEvents allowing you to handle them without implementation of another listener.
     *
    ...
    /**
     * Called when a {@link Command} catches a {@link Throwable} <b>during execution</b>. <p>This doesn't account for exceptions thrown during other pre-checks, and should not be treated as such! <p>An example of this misconception is via a {@link Command.Category} test:
     *
     * <pre>
    ...
    /**
     * Called when a {@link com.readonlydev.command.slash.SlashCommand SlashCommand} catches a {@link java.lang.Throwable Throwable} <b>during execution</b>. <p>This doesn't account for exceptions thrown during other pre-checks, and should not be treated as such! <p> The
     * {@link java.lang.NullPointerException NullPointerException} thrown will not be caught by this method!
     *
     * @param event     The CommandEvent that triggered the Command
     * @param command   The Command that was triggered
     * @param throwable The Throwable thrown during Command execution
    ...
    /**
     * Called when a {@link ContextMenu} catches a {@link java.lang.Throwable Throwable} <b>during execution</b>. <p>This doesn't account for exceptions thrown during other pre-checks, and should not be treated as such! <p> The {@link NullPointerException} thrown will not be caught by this method!
     *
