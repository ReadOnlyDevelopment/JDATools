...
import com.readonlydev.doc.annotation.docstandard.CommandInfo;
import com.readonlydev.doc.annotation.docstandard.Error;
import com.readonlydev.doc.annotation.docstandard.RequiredPermissions;

/**
 * An instance based documentation engine for bot commands written in JDA. <p>
 ...
 * represent. <p> The most basic usage of this can be shown below:
 *
 * <pre>
 ...
 * </pre>
 * 
 * <p> Note: This documentation system is <b>universal</b>, can be applied to
 * any command system that uses Class and/or Method based commands, and works in
 * any JVM language that supports annotations.
 *
 * @see com.readonlydev.doc.ConvertedBy
 * @see com.readonlydev.doc.annotation.DocConverter
 */
public class DocGenerator {
    ...
    /**
     * Gets a default DocGenerator with standard conversions loaded. <p> This is
     * the simplest way to get a prebuilt working CommandDoc generator with
     * standard annotations. <p> Additional annotations can be added using
     ...
     * the specified max-size. <p> Calls to
     ...
     * specified separator, and a cache with the specified max-size. <p> A
     * separator will be appended to the documentation returned by
     * {@link com.readonlydev.doc.DocGenerator#getDocFor(java.lang.Class)}
     * inbetween annotation conversions. <br> By default this is a double
     * newline ({@literal \n\n}). <p> Calls to
     * {@link com.readonlydev.doc.DocGenerator#getDocFor(java.lang.Class)},
     * {@link com.readonlydev.doc.DocGenerator#getDocFor(java.lang.reflect.Method)},
     * and
     ...
     * to this DocGenerator. <p> An example of a custom CommandDoc conversion
     ...
     *         {@link com.readonlydev.doc.ConvertedBy @ConvertedBy}, or an
     *         exception is thrown while instantiating the value said
     *         ConvertedBy annotation. <br> <b>NOTE:</b> that a DocConverter
     *         instantiation will fail if it's type parameter is the provided
     *         Annotation class.
     *
     * @return This DocGenerator
     */
    public <T extends Annotation> DocGenerator register(Class<T> type,
            Object... converterParams) {
        ConvertedBy convertedBy = type.getAnnotation(ConvertedBy.class);
        if (convertedBy == null) {
            throw new IllegalArgumentException(
                "Illegal annotation type! Not annotated with @ConvertedBy!");
        }
        ...
        try {
            ...
            if (converterParams.length > 0) {
                Class<?>[] tArray = Arrays
                    .stream(converterParams)
                    .map(Object::getClass)
                    .collect(Collectors.toList())
                    .toArray(new Class[converterParams.length]);
                instance = convertedBy
                    .value()
                    .getDeclaredConstructor(tArray)
                    .newInstance(converterParams);
            } else {
                instance = convertedBy.value().getConstructor().newInstance();
            }
        } catch (InstantiationException | IllegalAccessException
                | NoSuchMethodException | InvocationTargetException e) {
            throw new IllegalArgumentException("Instance of "
                    + convertedBy.value() + " could not be instantiated!",
                e);
        }
    ...
    /**
     * Registers a CommandDoc {@link java.lang.annotation.Annotation Annotation}
     * to this DocGenerator with the provided DocConverter. <p> This is not
     * recommended unless you for some reason want to pass an instance variable
     * to the DocConverter you are providing. <p> An example of a custom
            ...
            for (Class<? extends Annotation> key : map.keySet()) {
                ...
                if (docMultiple == null) {
                    ...
                    for (int i = 0; i < len; i++) {
                        ...
                        b
                            .append(docMultiple.prefixEach())
                            .append(((DocConverter<Annotation>) map.get(key))
                                .read(ans[i]));
                        if (i < (len - 1)) {
