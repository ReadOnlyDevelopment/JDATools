...
public class OrderedMenu extends Menu {
    ...
    OrderedMenu(EventWaiter waiter, Set<User> users, Set<Role> roles,
            long timeout, TimeUnit unit, Color color, String text,
            String description, List<String> choices,
            BiConsumer<Message, Integer> action, Consumer<Message> cancel,
     ...
     *         If <b>all</b> of the following are violated simultaneously: <ul>
     *         <li>Being sent to a
     *         {@link net.dv8tion.jda.api.entities.TextChannel
     *         TextChannel}.</li> <li>This OrderedMenu does not allow typed
     *         input.</li> <li>The bot doesn't have
     *         {@link net.dv8tion.jda.api.Permission#MESSAGE_ADD_REACTION
     *         Permission.MESSAGE_ADD_REACTION} in the channel this menu is
     *         being sent to.</li> </ul>
     */
    @Override
    public void display(MessageChannel channel) {
        ...
        if ((channel.getType() == ChannelType.TEXT) && !allowTypedInput
                && !((TextChannel) channel)
                    .getGuild()
                    .getSelfMember()
                    .hasPermission((TextChannel) channel,
                        Permission.MESSAGE_ADD_REACTION)) {
            throw new PermissionException(
                "Must be able to add reactions if not allowing typed input!");
        }
    ...
    public void display(Message message) {
        ...
        // Does not allow typed input
        // Does not have permission to add reactions
        if ((message.getChannelType() == ChannelType.TEXT) && !allowTypedInput
                && !message
                    .getGuild()
                    .getSelfMember()
    ...
    private void waitGeneric(Message m) {
        ...
        waiter.waitForEvent(GenericMessageEvent.class, e -> {
            // If we're dealing with a message reaction being added we return
            // whether it's
            // valid
            if (e instanceof MessageReactionAddEvent) {
                return isValidReaction(m, (MessageReactionAddEvent) e);
            }
            // If we're dealing with a received message being added we return
            // whether it's
            // valid
            ...
            if (e instanceof MessageReactionAddEvent) {
                ...
                if (event
                    ...
                    // The int provided in the success consumer is not indexed
                    // from 0 to number of
                    // choices - 1,
                    // but from 1 to number of choices. So the first choice will
                    // correspond to 1,
                    // the second
                    // choice to 2, etc.
                    action
    ...
    private void waitReactionOnly(Message m) {
        ...
        waiter.waitForEvent(MessageReactionAddEvent.class, e -> {
            ...
            if (e.getReaction().getReactionEmote().getName().equals(CANCEL)) {
                ...
                // The int provided in the success consumer is not indexed from
                // 0 to number of
                // choices - 1,
                // but from 1 to number of choices. So the first choice will
                // correspond to 1,
                ...
                action
                    .accept(m, getNumber(
                        e.getReaction().getReactionEmote().getName()));
            }
        }, timeout, unit, () -> cancel.accept(m));
    }
    ...
    private Message getMessage() {
        ...
        mbuilder
            .setEmbeds(new EmbedBuilder()
                .setColor(color)
                .setDescription(description == null ? sb.toString()
    ...
    private boolean isValidMessage(Message m, MessageReceivedEvent e) {
        ...
        return isValidUser(e.getAuthor(),
            e.isFromGuild() ? e.getGuild() : null);
    }
    ...
    private int getMessageNumber(String message) {
        if (useLetters) {
            ...
            return message.length() == 1
                    ? " abcdefghij".indexOf(message.toLowerCase(Locale.ROOT))
                    : -1;
        } else {
    ...
    /**
     * The {@link com.readonlydev.menu.Menu.Builder Menu.Builder} for an
     * {@link com.readonlydev.menu.OrderedMenu OrderedMenu}.
     */
    public static class Builder extends Menu.Builder<Builder, OrderedMenu> {
        ...
        /**
         * Builds the {@link com.readonlydev.menu.OrderedMenu OrderedMenu} with
         * this Builder.
         *
         ...
         *         was set.</li> <li>No choices were set.</li> <li>More than ten
         *         choices were set.</li> <li>No action
         *         {@link java.util.function.Consumer Consumer} was set.</li>
         *         <li>Neither text nor description were set.</li> </ul>
         */
        @Override
        public OrderedMenu build() {
            ...
            Checks
                .check(choices.size() <= 10,
                    "Must have no more than ten choices");
            Checks
                .check(selection != null, "Must provide an selection consumer");
            Checks
                .check((text != null) || (description != null),
                    "Either text or description must be set");
            return new OrderedMenu(waiter, users, roles, timeout, unit, color,
                text, description, choices, selection, cancel, useLetters,
                allowTypedInput, addCancel);
        }
        ...
        /**
         * Sets the builder to build an {@link com.readonlydev.menu.OrderedMenu
         * OrderedMenu} using letters for ordering and reactions (IE: A, B, C,
         * etc.). <br> As a note - by default the builder will use
         * <b>numbers</b> not letters.
         *
         * @return This builder
         */
        ...
        /**
         * Sets the builder to build an {@link com.readonlydev.menu.OrderedMenu
         * OrderedMenu} using numbers for ordering and reactions (IE: 1, 2, 3,
         * etc.).
         *
         * @return This builder
         */
        ...
        /**
         * If {@code true}, {@link net.dv8tion.jda.api.entities.User User}s can
         * type the number or letter of the input to make their selection, in
         * addition to the reaction option.
         *
         * @param allow
         *        {@code true} if raw text input is allowed, {@code false} if it
         *        is not
         *
         * @return This builder
         */
        ...
        /**
         * If {@code true}, adds a cancel button that performs the timeout
         * action when selected.
         *
         * @param use
         *        {@code true} if the cancel button should be shown,
         ...
         * {@link com.readonlydev.menu.OrderedMenu OrderedMenu} is built. <p>
         ...
         * {@link net.dv8tion.jda.api.entities.MessageEmbed MessageEmbed}. <br>
        ...
        /**
         * Sets the {@link java.util.function.BiConsumer BiConsumer} action to
         * perform upon selecting a option.
         *
         * @param selection
         ...
         * the {@link com.readonlydev.menu.OrderedMenu OrderedMenu} is
         * cancelled.
         *
         * @param cancel
        ...
        public Builder addChoice(String choice) {
            Checks
                .check(choices.size() < 10, "Cannot set more than 10 choices");
        ...
        /**
         * Adds the String choices. <br> These correspond to the button in order
         * of addition.
         *
         * @param choices
        ...
        /**
         * Sets the String choices. <br> These correspond to the button in the
         * order they are set.
         *
         * @param choices
