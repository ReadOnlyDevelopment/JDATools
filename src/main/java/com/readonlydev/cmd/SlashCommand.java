 ...
 * <p>
 * This intends to mimic the {@link Command command} with minimal breaking
 * changes, to make migration easy and smooth.
 * </p>
 * <p>
 * Breaking changes are documented <a href=
 * "https://github.com/Chew/JDA-Chewtils/wiki/Command-to-SlashCommand-Migration">here</a>.
 * </p>
 ...
 * steps:
 * <ul>
 * <li>{@link SlashCommand#run(SlashCommandEvent, CommandClient) run} - The
...
public abstract class SlashCommand extends Command {
	/**
	 * This option is deprecated in favor of {@link #enabledRoles} Please replace
	 * this with this.enabledRoles = new String[]{Roles}; While this check is still
	 * done, it's better to let Discord do the work.<br>
	 * This deprecation can be ignored if you intend to support normal and slash
	 * commands.
	 */
	@Deprecated
	...
	/**
	 * The list of user IDs who can use this Slash Command. Because command
	 * privileges are restricted to a Guild, these will not take effect for Global
	 * commands.<br>
	 * This is useless if {@link #defaultEnabled} isn't false.
	 */
	...
	/**
	 * The list of role IDs who cannot use this Slash Command. Because command
	 * privileges are restricted to a Guild, these will not take effect for Global
	 * commands.<br>
	...
	/**
	 * The list of user IDs who cannot use this Slash Command. Because command
	 * privileges are restricted to a Guild, these will not take effect for Global
	 * commands.<br>
	 * This is useless if {@link #defaultEnabled} isn't true.
	 */
	...
	/**
	 * Whether this command is disabled by default. If disabled, you must give
	 * yourself permission to use it.<br>
	 ...
	 * {@code /<parent name>
	 * <child name>}. This is synonymous with sub commands. Additionally,
	 * sub-commands cannot have children.<br>
	 */
	protected SlashCommand[] children = new SlashCommand[0];

	/**
	 * The ID of the server you want guildOnly tied to. This means the slash command
	 * will only work and show up in the specified Guild. If this is null, guildOnly
	 * will still be processed, however an ephemeral message will be sent telling
	 * them to move.
	 *
	 * @deprecated This will be removed in favor of
	 * {@link CommandClientBuilder#forceGuildOnly(String)}. Please use
	 * that instead.
	 *
	 ...
	 * It's important the instantiations are the same across children if you intend
	 ...
	 * {@link SlashCommand#run(SlashCommandEvent, CommandClient)
	 * #run(CommandEvent)}.
	 *
	 * @param event The {@link SlashCommandEvent SlashCommandEvent} that triggered
	 * this Command
	 */
	 ...
	 * #run(CommandEvent)}. <b> Because this is a SlashCommand, this is called, but
	 * does nothing. You can still override this if you want to have a separate
	 * response for normal [prefix][name]. Keep in mind you must add it as a Command
	 * via {@link CommandClientBuilder#addCommand(Command)} for it to work properly.
	 * </b>
	 *
	...
	/**
	 * Runs checks for the {@link SlashCommand SlashCommand} with the given
	 * {@link SlashCommandEvent SlashCommandEvent} that called it. <br>
	 ...
	 * @param client The CommandClient for checks and stuff
	 */
	public final void run(SlashCommandEvent event, CommandClient client) {
		...
		if (requiredRole != null) {
			if (!(event.getChannelType() == ChannelType.TEXT) || event.getMember().getRoles().stream()
					.noneMatch(r -> r.getName().equalsIgnoreCase(requiredRole))) {
				terminate(event, client.getError() + " You must have a role called `" + requiredRole + "` to use that!",
						client);
				return;
			}
		}
			...
			for (Permission p : botPermissions) {
				...
				if (p.isChannel()) {
					if (p.name().startsWith("VOICE")) {
						...
						if (vc == null) {
							terminate(event, client.getError() + " You must be in a voice channel to use that!",
									client);
							return;
						} else if (!selfMember.hasPermission(vc, p)) {
							terminate(event, String.format(botMissingPermMessage, client.getError(), p.getName(),
					...
					if (!selfMember.hasPermission(p)) {
						terminate(event, String.format(botMissingPermMessage, client.getError(), p.getName(), "server"),
								client);
						return;
					}
				}
	...
	/**
	 * Gets the enabled roles for this Slash Command. A user MUST have a role for a
	 * command to be ran.
	 *
	...
	/**
	 * Gets the enabled users for this Slash Command. A user with an ID in this list
	 * is required for the command to be ran.
	 *
	...
	/**
	 * Gets the disabled roles for this Slash Command. A user with this role may not
	 * run this command.
	 *
	 * @return a list of String role IDs
	 */
	...
	/**
	 * Gets the disabled users for this Slash Command. Uses in this list may not run
	 * this command.
	 *
	 * @return a list of String user IDs
	 */
	...
	/**
	 * Whether or not this command is enabled by default. If disabled by default,
	 * you MUST enable {@link #enabledRoles roles} or {@link #enabledUsers users} to
	 * access it. This does NOT hide it, it simply appears greyed out.
	 *
	 * @return a list of String user IDs
	 */
	...
	/**
	 * Builds CommandData for the SlashCommand upsert. This code is executed when we
	 * need to upsert the command.
	...
	/**
	 * Builds CommandPrivilege for the SlashCommand permissions. This code is
	 * executed after upsertion to update the permissions. <br>
	 * <b>The max amount of privilege is 10, keep this in mind.</b>
	 * Useful for manual upserting.
	 *
	 * @param client the command client for owner checking. if null, owner checks
	 * won't be performed
	 *
	...
	public String getCooldownKey(SlashCommandEvent event) {
		...
		case USER_GUILD:
			...
			? cooldownScope.genKey(name, event.getUser().getIdLong(), event.getGuild().getIdLong())
					: CooldownScope.USER_CHANNEL.genKey(name, event.getUser().getIdLong(),
